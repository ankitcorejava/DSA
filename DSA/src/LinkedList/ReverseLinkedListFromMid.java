package LinkedList;


public class ReverseLinkedListFromMid {

	private static class Node {
		private int value;
		private Node next;

		public Node(int value) {
			this.value = value;
			this.next = null;
		}

		public Node() {
			this.value = 0;
			this.next = null;
		}

	}

	private static class LinkedList {
		private Node head;
		private Node tail;
		private int size;

		public LinkedList() {
			this.head = null;
			this.tail = null;
			this.size = 0;
		}

		// Get Size
		public int size() {
			return this.size;
		}

		// Add Node at start
		public void addNodeAtStart(int value) {
			Node node = new Node(value);
			if (size() == 0) {
				this.head = node;
				this.tail = node;
				this.size++;
			} else {
				node.next = this.head;
				this.head = node;
				this.size++;
			}
		}

		// Add Node at given index
		public void addNodeAtIndex(int index, int value) {
			if (index >= size() || index < 0) {
				System.out.println("Invalid Index");
				return;
			}

			Node node = new Node(value);
			if (size() == 0) {
				addNodeAtStart(value);
				return;
			} else {
				int idx = 0;
				Node previous = null;
				Node current = this.head;
				while (idx != index) {
					previous = current;
					current = current.next;
					idx++;
				}
				previous.next = node;
				node.next = current;
				this.size++;
			}

		}

		// Find Mid
		public void findMid() {
			Node slow = this.head;
			Node fast = this.head;

			while (fast.next != null && fast.next.next != null) {
				fast = fast.next.next;
				slow = slow.next;
			}
			System.out.print(slow.value);

		}

		// Display

		public void display() {
			Node current = this.head;
			while (current != null) {
				System.out.print("-> " + current.value);
				current = current.next;
			}
		}

		public void reverseLinkedListPointerIterative(int left, int right) {
			Node current = this.head;
			Node next = this.head;
			Node previous = null;
			while (current != null) {
				next = current.next;
				current.next = previous;
				previous = current;
				current = next;
			}
			this.tail = this.head;
			this.head = previous;

		}

		public Node reverseBetween(int left, int right) {

			Node current = this.head;
			//Node tail = this.head;
			Node previous = null;
			Node nextNode = null;
			Node nexts = null;
			int idx = 1;

			// Create two dummy Nodes :
			Node dummy1 = new Node(1);
			Node dummy2 = new Node(2);

			Node dummyHead1 = dummy1;
			Node dummyHead2 = dummy2;

			// Part:1 Run loop till left and point dummy1 to 1st part;
			while (idx < left && current!= null) {
				
				dummyHead1.next = current;
				dummyHead1 = dummyHead1.next;
				current = current.next;
				idx++;
			}
			dummyHead1.next = null;
			nexts =current;

			while (idx <= right && current!= null) {
				nextNode = current.next;
				current.next = previous;
				//tail.next = current;
				previous = current;
				current = nextNode;
				
				idx++;
			}

			dummyHead1.next = previous;
			
			return dummyHead1;
		}
	}

	public static void main(String[] args) {
		LinkedList linkedList = new LinkedList();
		linkedList.addNodeAtStart(10);
		linkedList.addNodeAtStart(20);
		linkedList.addNodeAtStart(30);
		linkedList.addNodeAtStart(90);
		linkedList.addNodeAtIndex(2, 40);
		linkedList.addNodeAtIndex(2, 50);
		// linkedList.addNodeAtIndex(-1,420);
		linkedList.display();
		System.out.println();
		linkedList.findMid();
		// linkedList.reverseLinkedListPointerIterative(2, 4);
		System.out.println();
		linkedList.display();
		linkedList.reverseBetween(3, 4);
		System.out.println();
		linkedList.display();

	}

}
